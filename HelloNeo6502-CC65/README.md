...
Note: I have made significant progress on a Neo6502 graphics library and expect to release it soon.
...
- Includes a Neo6502 API library and 'none' library (the build target that CC65 uses when none is specified)
- Neo6502 is not a target in CC65, so using a custom 'none' library is a simple and effective workaround
- Graphics library is well underway "comming soon"

# To Build
The project is built either using build.bat (or buildrun.bat) files or build.sh files. These files must first be edited to add the indicated paths to CC65 and to the latest Neo6502 build tools and libraries. Use the batch files in HelloNeo6502-CC65 as a template. A project also needs a CC65 linker config file. "Use neo6502.cfg"in the sample directory.

The project produces an executable file "hello.neo" and copies it to the "storage" directory. To run this file in the emulator from the command line use:

<path to emulator>neo.exe keys "hello.neo" cold
